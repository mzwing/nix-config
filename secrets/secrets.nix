let
  mzwing = "ssh-rsa  lockinwise lolite@MZWING-PC";
  mzwingDoSgp = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBt5rrGbMYQ1c4dtRrzJpGX5RDyKh5/c5ABdnSgfoGDJ root@mzwing-do-sgp";
  mzwingUpcloudSg = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFN6u5TVLoI3OzCfCqjVYThl+4UcAXZ8noRJO05aoHcL root@mzwing-upcloud-sg";
  doSgpRecipients = [
    mzwing
    mzwingDoSgp
  ];
  upcloudSgRecipients = [
    mzwing
    mzwingUpcloudSg
  ];
in {
  "cliproxyapiplus/api-key.age".publicKeys = [mzwing];
  "cliproxyapiplus/remote-secret-key.age".publicKeys = [mzwing];
  "do-sgp/network/private.age".publicKeys = doSgpRecipients;
  "do-sgp/network/public.age".publicKeys = doSgpRecipients;
  "upcloud-sg/network/public-ipv4.age".publicKeys = upcloudSgRecipients;
  "upcloud-sg/network/public-ipv6.age".publicKeys = upcloudSgRecipients;
  "upcloud-sg/network/utility-ipv4.age".publicKeys = upcloudSgRecipients;
}
