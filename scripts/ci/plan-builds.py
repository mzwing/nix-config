...
Reads ALL_TARGETS (a JSON array of {name, system, drvPath, outputPath}
...
outputs to GITHUB_OUTPUT:
  ...
  targets         JSON array of host targets not found in any binary cache
  devenv_systems  JSON array of systems whose devenv shell needs rebuilding
  builders        {"include": [...]} matrix for the builder job
  has_builds      "true" if anything needs building, otherwise "false"
"""
...
USER_AGENT = "nix-config-ci-plan-builds"
...
def main() -> None:
    ...
    devenv_outputs: dict[str, list[str]] = json.loads(os.environ["DEVENV_OUTPUTS"])
    ...
    with ThreadPoolExecutor() as executor:
        ...
        targets = [
            target for target, hit in zip(all_targets, hits, strict=True) if not hit
        ]

        devenv_entries = [
            (system, output_path)
            for system, output_paths in devenv_outputs.items()
            for output_path in output_paths
        ]
        devenv_hits = executor.map(probe, (path for _, path in devenv_entries))
        devenv_systems = sorted(
            {
                system
                for (system, _), hit in zip(devenv_entries, devenv_hits, strict=True)
                if not hit
            }
        )

    active_systems = sorted(
        {target["system"] for target in targets} | set(devenv_systems)
    )
    ...
    outputs = {
        ...
        "devenv_systems": json.dumps(devenv_systems, separators=(",", ":")),
        "has_builds": "true" if targets or devenv_systems else "false",
